...
path = 'G:/edgedownload/test_256/test8000/'
k1_top = 1e-4
k1_down = 1e-6
...
if __name__ == "__main__":
    ...
    for files in img_list:
        ...
        print(files)
        srcImg = cv.imread('G:/edgedownload/test_256/test8000/' + files)
        srcImg = cv.resize(srcImg, (400, 400))
        dstImg, cutImg, source_Img = create_fish(srcImg, k1, k2, k3, k4)

        cv.imwrite('../dataset/data/test/' + str(num) + '.jpg', cutImg)
        cv.imwrite('../dataset/gt/test/' + str(num) + '.jpg', source_Img)
        num = num + 1